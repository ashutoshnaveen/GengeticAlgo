...
function [Pi,Qi,Pg,Qg,Pl,Ql,Lpij,Lqij] = loadflow1(x,nb,V,del,BMva)
Y = ybusppg(nb);                % Calling Ybus program..
%Y = Ybus(nb);
DGa;Pdg = zeros(38,1);dis = [2 5 12 14 19 22 31 32];
Pdg(DG)=x(1:10); discharge = x(11:end);
disch = zeros(38,2);k = 1;
for i = 1:38
    if i == dis(k)
        k= k+1;
        disch(i,2)=discharge(k-1);
    end
    if k > 8
        break;
    end
...
busd = busdatas19(nb);            % Get busdatas..
